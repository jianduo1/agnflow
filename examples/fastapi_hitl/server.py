...
from .main import create_feedback_flow # PocketFlow imports
...
# Structure: task_id -> {"state": dict, "status": str, "task_obj": asyncio.Task | None}
...
async def run_flow_background(task_id: str, flow:Flow, state: Dict[str, Any]):
    """Runs the flow in background, uses queue in state for SSE."""
    ...
    if not queue:
        print(f"ERROR: Task {task_id} missing sse_queue in state store!")
    ...
    try:
        ...
        # Determine final status based on state state after flow completion
        ...
        if task_id in tasks:
            ...
            if final_status == "completed":
                final_update["final_result"] = state.get("final_result")
            elif error_message:
...
async def submit_task(
    ...
    background_tasks.add_task(run_flow_background, task_id, flow, state)
...
async def stream_status(task_id: str):
    ...
    if task_id not in tasks or "sse_queue" not in tasks[task_id]["state"]:
         raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Task or queue not found")

    queue = tasks[task_id]["state"]["sse_queue"]
...
if __name__ == "__main__":
    ...
    import uvicorn
    uvicorn.run("cookbook.pocketflow-fastapi-hitl.server:app", host="0.0.0.0", port=8000, reload=True)
