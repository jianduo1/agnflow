...
def user_validation(user_id, state):
    ...
    if user_id in state.get("valid_users", []):
        return "valid", {"user_validated": True, "validation_time": time.time()}
    else:
        return "invalid", {"user_validated": False, "error": "用户不存在"}
...
async def async_data_fetch(user_id, state):
    ...
    user_data = {
        "user_id": user_id,
        "profile": {"name": f"用户{user_id}", "level": "VIP"},
...
def create_mixed_injection_workflow():
    ...
    # 连接工作流
    validation_node >> {
        "valid": process_node >> fetch_node >> response_node,
        "invalid": error_node
    }
